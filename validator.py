...
def validate_domains():
    ...
    domains = sorted(set(d for d in domains if valid_domain(d)))
    print(f"INPUT={len(domains)} THREADS={THREADS}")
    good = set()
    with ThreadPoolExecutor(max_workers=THREADS) as ex:
        futures = [ex.submit(resolve_domain, domain) for domain in domains]
